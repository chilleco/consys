"""Base ConSys model definitions.

This module contains the descriptor-driven field system (`Attribute`) and the
`BaseModel` implementation that powers every ConSys ORM model described in the
ConSys ORM Handbook.  The code here defines how objects talk to MongoDB,
including change detection, optimistic locking, and partial-field semantics.
"""
...
class Attribute:
    """Descriptor implementing ConSys field semantics.

    Each declared attribute participates in validation, transformation, and
    serialization.  The descriptor mirrors the rules documented in the handbook:
    strict type casting, optional `checking` hooks, and lazy defaults for both
    root documents and embedded subobjects.
    """
    ...
    def __set_name__(self, instance, name):
        """Bind the descriptor to a concrete attribute name."""

        self.name = name

    def __get__(self, instance, owner):
        """Return raw value for owners and hydrated/cached value for instances."""
    ...
    def __set__(self, instance, value) -> None:
        """Validate and normalize assignment according to descriptor settings."""
    ...
    def __delete__(self, instance):
        """Drop cached value so the default pipeline can run again."""
...
class BaseModel:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        The constructor enforces descriptor definitions, preloads `_loaded_values`
        when a document comes from Mongo, and applies the autogenerated timestamps
    ...
    def __setattr__(self, name, value):
        """Block dynamic attribute creation and delegate to descriptors."""
    ...
    def __getitem__(self, name):
        """Mapping-style getter mirroring descriptor access."""
        return getattr(self, name)

    def __setitem__(self, name, value):
        """Mapping-style setter that respects descriptor validation."""
        setattr(self, name, value)

    def __repr__(self):
        """Render a debug-friendly representation (partial-aware)."""
    ...
    def __enter__(self):
        """Context manager hook for ergonomic `with Model(...) as obj` usage."""

        return self

    def __exit__(self, exc_type, exc_value, traceback):
        """Context manager hook; no-op because persistence is explicit."""

    def __iter__(self):
        """Iterate over public fields as `(key, value)` pairs."""
    ...
    @classmethod
    def _next_id(cls):
        """Compute the next integer `id` using the backing Mongo collection."""
    ...
    def _is_default(self, name):
        """Check whether the current attribute matches the descriptor default."""
    ...
    def _is_subobject(data):
        """Return True when value looks like a ConSys embedded subobject.

        Subobjects are stored as dicts within arrays but always include their own
        `id`.  This helper lets `_get_changes` split `$push/$pull` updates so that
        embedded lists follow the rules from the handbook.
        """
    ...
    def _get_changes(self, data):
        """Compare current payload with `_loaded_values` and bucket mutations."""
